...
class UcsrTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lr_tensor = torch.as_tensor(np.array(lr_image).astype('float') / 255.0).permute(2, 0, 1) #### 억까 심하네.. transforms.ToTensor()대신 쓰기
        ...
        hr_tensor = torch.as_tensor(np.array(hr_image).astype('float') / 255.0).permute(2, 0, 1)
...
class UcsrValidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hr_image = Image.open(self.valid_lr_path_list[idx]).convert("RGB")
        hr_tensor = torch.as_tensor(np.array(hr_image).astype('float') / 255.0).permute(2, 0, 1)
        # print(f'***{hr_tensor.shape}') 
